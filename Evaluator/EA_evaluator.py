...
from Evaluator.Evaluartor_Interface import evaluator
from Evaluator.Utils import dataset, train
...
class EA_eval(evaluator):
    ...
    def eval_model(self, individual):
        ...
        trainset, validset = dataset.build_cifar10(self.data_path, self.cutout_size, self.num_work, self.train_batch_size, self.eval_batch_size, self.split_train_for_valid)
        ...
        train_criterion, eval_criterion, optimizer, scheduler = train.build_train_utils(model, epochs=self.epochs)
        # train procedures
        step = 0
        for epoch in range(self.epochs):
            train_loss, train_top1, train_top5, step = train.train(trainset, model, optimizer, step, train_criterion, device)
            print("[Epoch {0:>5d}] [Train] loss {1:.3f} error Top1 {2:.2f} error Top5 {3:.2f}".format(epoch, train_loss, train_top1, train_top5))
            scheduler.step()
        
        valid_loss, valid_top1, valid_top5 = train.valid(validset, model, eval_criterion, device)
        print("[Epoch {0:>5d}] [Valid] loss {1:.3f} error Top1 {2:.2f} error Top5 {3:.2f}".format(epoch, valid_loss, valid_top1, valid_top5))
